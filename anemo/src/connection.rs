...
use crate::{
    config::RetryConfig,
    error::{RecvError, SendError, SerializationError, StreamError},
    wire_msg::WireMsg,
};
use bytes::Bytes;
use futures::{
...
use std::{fmt, net::SocketAddr, pin::Pin, sync::Arc, task};
    ...
    fn poll_next(
        ...
        task::Poll::Ready(match next.transpose() {
            ...
            Err(error) => {
                ...
                if matches!(error, ConnectionError::TimedOut) {
...
mod tests {
    ...
    use crate::config::ToPKCS8;
    use crate::{config::MachineConfig, error::SendError, wire_msg::WireMsg};
    use bytes::Bytes;
    ...
    fn dalek_kp() -> ed25519_dalek::Keypair {
    ...
    async fn basic_usage() -> Result<()> {
        ...
        let public_key_bytes = <Ed25519 as Certifiable>::public_key_to_spki(&public_key);
        let config = MachineConfig::<Ed25519>::new(kp, "localhost", 8080, None)?;

        let server_config = config.server_config_for(&public_key_bytes)?;
        let client_config = config.client_config_for(&public_key_bytes)?;

        let (mut peer1, _peer1_incoming) =
            QuinnEndpoint::server(server_config.clone(), local_addr())?;
        peer1.set_default_client_config(client_config);

        let (peer2, peer2_incoming) = QuinnEndpoint::server(server_config.clone(), local_addr())?;

        {
            let (p1_tx, mut p1_rx) = Connection::new(
                peer1.clone(),
                None,
                peer1.connect(peer2.local_addr()?, "localhost")?.await?,
            );
    ...
    async fn benign_connection_loss() -> Result<()> {
        ...
        let public_key = kp.public;
        let public_key_bytes = <Ed25519 as Certifiable>::public_key_to_spki(&public_key);
        let mut config = MachineConfig::<Ed25519>::new(kp, "localhost", 8080, None)?;
